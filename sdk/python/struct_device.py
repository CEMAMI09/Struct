"""Struct UDP device client. Standard library only; plaintext + HMAC authentication.

For payload confidentiality use the C/ESP32 or Node encryption implementation,
or a secure network. Never confuse HMAC with encryption.
"""
import hashlib
import hmac
import ipaddress
...
def build_frame(key_id, api_secret, schema_version, payload, *, confirmed=False,
    ...
    body = struct.pack('<B16sBI12s', 3 if confirmed else 2, key_id.encode('ascii'),
...
class StructClient:
    def __init__(self, host, key_id, api_secret, port=8081):
    ...
    def send(self, schema_version, payload, *, confirmed=False, max_retries=1,
        ...
        try:
            return self._send(schema_version, payload, confirmed, max_retries, retry_ms, budget_ms)
        finally:
            self._lock.release()

    def _send(self, version, payload, confirmed, max_retries, retry_ms, budget_ms):
            ...
            while time.monotonic() < deadline:
                ...
                while time.monotonic() < next_send:
                    ...
                    if verify_receipt(receipt, frame[-32:], self.api_secret):
                        return {'status': 'committed', 'attempts': attempts, 'duplicate': receipt[36] == 1}
