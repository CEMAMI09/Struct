...
Struct is an ultra-lightweight IoT gateway for battery-constrained fleets. Devices send packed C/C++ structs over raw TCP instead of verbose JSON. The gateway authenticates each device, resolves its schema version, optionally decrypts the payload, stores telemetry in Supabase, and routes clean JSON to downstream systems.

```text
ESP32 / MCU
   │  [16B API key][1B schema version][packed little-endian payload]
   ▼
Node TCP gateway (:8080)
...
- Supported field types are `float32`, `int32`, `uint8`, and `boolean`.
...
Paid plans grow and shrink with active device count:

- If a user creates a device while already at the current allowance, Struct raises the Stripe subscription item quantity by one and applies prorations.
- If a user deletes a device, Struct reduces quantity to the amount required for the remaining fleet, but never below that plan's floor.
...
`tcp-server/server.js` uses Node's native `net` server and listens on `TCP_PORT` (default `8080`).

For each frame it:

1. reads the 16-byte API key and one-byte schema version;
2. loads the device and matching immutable schema;
3. determines the exact frame length from the schema;
4. optionally verifies/decrypts ChaCha20-Poly1305;
5. parses the packed little-endian payload;
6. inserts telemetry and updates `last_seen`;
7. asynchronously fans out matching destination webhooks;
8. delivers queued downlinks on the same socket.

The gateway accepts multiple complete frames on a connection and buffers partial TCP chunks until a complete frame is available.

### Ingestion protections

- Per-IP connection limit: 10 connections/second by default.
- Per-API-key payload limit: 50 payloads/minute by default.
- Rate-limited devices receive the custom downlink code `ff e1` before disconnect.
- Encrypted payload timestamps must be within `TCP_REPLAY_SKEW_SEC` (default 60 seconds).
- Duplicate encrypted nonces are rejected during the replay window.
- Destination requests time out after eight seconds.

The rate-limit state and replay cache are in memory and therefore local to each gateway process.
...
```text
Offset 0      16 bytes   ASCII API key, no NUL terminator
Offset 16      1 byte    schema version (1-255)
Offset 17...   N bytes   packed little-endian fields for that version
```
...
Packed type sizes: `float32=4`, `int32=4`, `uint8=1`, `boolean=1`.
...
| `tcp-server/` | Node TCP ingestion, parser, crypto, replay protection, rate limits, destination delivery, and downlinks |
...
| `dummy-device.js` | Local Node device simulator |
...
6. Put the API key into `dummy-device.js` or `test_device.cpp`.
7. Start the TCP gateway and send a frame.
...
# Send the local sample frame
node dummy-device.js
```
...
- The TCP gateway is not TLS. Use a private network, VPN, secure tunnel, or TLS-terminating proxy when confidentiality of the API key/header is required.
